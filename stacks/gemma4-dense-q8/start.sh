#!/bin/sh
##############################################################################
# Docker Startup Script for Gemma 4 31B (Dense) Q8_0 on 2x Tesla P40
#
# Backend: upstream ggml-org/llama.cpp (gemma4-server-local image)
#   NOT ik_llama.cpp — see gemma4-moe-q8/build.sh for rationale.
#
# NOTE: 31B Q8_0 is ~31GB — tight fit on 48GB total VRAM. Context is limited
# to 32k to leave headroom for KV cache + compute buffers. If this OOMs,
# drop to 16k or use the Q6_K variant instead.
##############################################################################

MODEL="/models/gemma-4-31b-it/gemma-4-31B-it-Q8_0.gguf"
CTX=${CTX_SIZE:-32768}
THREADS=32
...
echo "[startup] Quant: Q8_0 (~31GB — TIGHT fit, 32k context max)"
...
echo "[startup] Starting llama-server with Gemma 4 31B Q8_0..."
